// Dafny program RL_No_feedback.dfy compiled into C#
...
[assembly: DafnyAssembly.DafnySourceAttribute(@"// dafny 4.10.1.0
// Command Line Options: /mnt/shared/gpfs/home/manvij2/journal_phase/test_folder_one/Dafny(55)/RL_No_feedback.dfy
// RL_No_feedback.dfy
...
lemma SequenceArrayEquality(xs: seq<int>, a: array<int>)
  requires a.Length == |xs|
  requires forall i: int {:trigger xs[i]} {:trigger a[i]} :: 0 <= i < |xs| ==> a[i] == xs[i]
  ensures forall i: int {:trigger xs[i]} {:trigger a[i]} :: 0 <= i < a.Length ==> a[i] == xs[i]
  decreases xs, a
{
}
")]
