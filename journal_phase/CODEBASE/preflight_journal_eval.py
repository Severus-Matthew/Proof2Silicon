...
def main():
    ...
    if not a.skip_api and not failures:
        for raw in a.models:
            ...
            try:
                text,_,_=call_model(spec, 'Reply with exactly OK.', 'OK', max_tokens=8,
                                    reasoning='low' if spec.provider=='openai' else None,
                                    temperature=0.0, retries=2)
                print(f'model {raw}: OK ({text[:60]!r})')
            except Exception as e:
                print(f'model {raw}: FAILED ({e})')
                failures.append(f'model unavailable: {raw}: {e}')
    if failures:
        print('\nPREFLIGHT FAILED:')
