...
# def run_slm(model, tokenizer, prompt, filename):
...
#         # Save interaction
#         save_dir = "/mnt/shared/gpfs/home/manvij2/UIUC_PROJ_2/test_outputs/slm_new"
#         os.makedirs(save_dir, exist_ok=True)
        
#         with open(os.path.join(save_dir, f"slm_interaction_{filename}_{time.strftime('%Y%m%d_%H%M%S')}.json"), "w") as f:
#             json.dump({
#                 "prompt": prompt,
#                 "response": response
#             }, f, indent=2)
...
#         with open(os.path.join(save_dir, f"llm_interaction_gemini_{filename}_{time.strftime('%Y%m%d_%H%M%S')}.json"), "w") as f:
#             json.dump({
#                 "prompt": full_prompt,
...
# def run_llm_chatgpt4o(prompt, filename):
#     time.sleep(30)
#     """Run the Gemini model to generate code."""
#     try:
...
#         with open(os.path.join(save_dir, f"llm_interaction_chagpt4o_{filename}_{time.strftime('%Y%m%d_%H%M%S')}.json"), "w") as f:
#             json.dump({
#                 "prompt": full_prompt,
...
# def run_llm_chatgpto1(prompt, filename):
#     time.sleep(30)
#     try:
...
#         with open(os.path.join(save_dir, f"llm_interaction_chagpto1_{filename}_{time.strftime('%Y%m%d_%H%M%S')}.json"), "w") as f:
#             json.dump({
#                 "prompt": full_prompt,
...
# def run_llm_qwen7b(prompt, filename):
#     time.sleep(30)
...
#         save_dir = "/mnt/shared/gpfs/home/manvij2/UIUC_PROJ_2/test_outputs/llm_new"
#         os.makedirs(save_dir, exist_ok=True)
#         with open(os.path.join(save_dir, f"llm_interaction_qwen7b_{filename}_{time.strftime('%Y%m%d_%H%M%S')}.json"), "w") as f:
#             json.dump({
#                 "prompt": full_prompt,
#                 "response": response
#             }, f, indent=2)
...
# def run_llm_qwen(prompt, filename):
#     time.sleep(30)
#     """Run the Qwen model to generate code."""
#     try:
...
#         save_dir = "/mnt/shared/gpfs/home/manvij2/UIUC_PROJ_2/test_outputs/llm_new"
#         os.makedirs(save_dir, exist_ok=True)
#         with open(os.path.join(save_dir, f"llm_interaction_qwen_{filename}_{time.strftime('%Y%m%d_%H%M%S')}.json"), "w") as f:
#             json.dump({
#                 "prompt": full_prompt,
#                 "response": response
#             }, f, indent=2)
...
#     slm_response = run_slm(slm_model, tokenizer, short_prompt, entry)
#     if llm_type == "gemini":
#         llm_response = run_llm(slm_response, entry)
#     elif llm_type == "qwen":
#         llm_response = run_llm_qwen(slm_response, entry)
#     elif llm_type == "gpt4o":
#         llm_response = run_llm_chatgpt4o(slm_response, entry)
#     elif llm_type == "o1mini":
#         llm_response = run_llm_chatgpto1(slm_response, entry)
#     elif llm_type == "qwen7b":
#         llm_response = run_llm_qwen7b(slm_response, entry)
#     else:
...
#         slm_response = run_slm(slm_model, tokenizer, error_prompt, entry)
#         if llm_type == "gemini":
#             llm_response = run_llm(slm_response, entry)
#         elif llm_type == "qwen":
#             llm_response = run_llm_qwen(slm_response, entry)
...
#     output_dir = "/mnt/shared/gpfs/home/manvij2/UIUC_PROJ_2/test_outputs/Test_with_trained"
#     os.makedirs(output_dir, exist_ok=True)
#     entries = [entry for entry in os.listdir(root_dir) if os.path.isdir(os.path.join(root_dir, entry))]
#     llm_types = ["qwen7b", "qwen"]
...
OUTPUT_DIR = "/u/mjha1/Proof2Silicon/journal_phase/baseline_outputs_trained_lora7"

SLM_MODEL_NAME = "Qwen/Qwen2.5-1.5B-Instruct"
SLM_LORA_PATH = "/u/mjha1/Proof2Silicon/journal_phase/checkpoints_4/run_CHK/final_adapter"
...
MAX_FEEDBACK_ATTEMPTS = 7
DAFNY_TIMEOUT_SEC = 120
SLEEP_BETWEEN_API_CALLS = 2

OFFLOAD_FOLDER = "/u/mjha1/Proof2Silicon/journal_phase/model_offload_test_lora"
...
GLOBAL_SLM_MODEL = None
GLOBAL_SLM_TOKENIZER = None
...
def create_short_prompt(task: str) -> str:
    return (
        "You are an expert in understanding Dafny code and explaining it in detailed, structured steps "
        "to another agent. Your explanations should be clear, thorough, and logically organized so that the agent "
        "can effectively use them to write correct and error-free Dafny code.\n\n"
        "Given the following task, break down the required Dafny program into:\n"
        "1. Step-by-step reasoning about the logic and specification.\n"
        "2. Relevant Dafny constructs and their usage, such as preconditions, postconditions, invariants, asserts, and decreases clauses.\n"
        ...
        f"Here is the task:\n{task}\n\n"
        "Return only the instruction for the coding agent."
    )
...
def create_error_prompt(task: str, code: str, error: str, reward: str) -> str:
    return (
        ...
        "to another agent. Your explanations should be clear, thorough, and logically organized so that the agent "
        "can effectively use them to write correct and error-free Dafny code.\n\n"
        "A previous LLM-generated Dafny program based on your earlier instruction failed.\n"
        "Generate a revised instruction for the coding agent.\n\n"
        "Your revised instruction should include:\n"
        "1. Step-by-step reasoning about the logic and specification and ways to solve this error.\n"
        "2. Relevant Dafny constructs and their usage, such as preconditions, postconditions, invariants, asserts, and decreases clauses.\n"
        "3. Potential edge cases and how to handle them.\n"
        "4. Specific guidance to avoid syntax errors, undefined variables, type errors, out-of-bounds errors, weak invariants, missing proof steps, and logic/spec mismatches.\n"
        "5. Suggestions to help the verifier prove the program.\n\n"
        "6. avoid recursion in the code.\n"
        f"Here is the task:\n{task}\n\n"
        f"Below is the errored code:\n{code}\n\n"
        f"Below is the error message:\n{error}\n\n"
        f"Below is the reward for the previous attempt:\n{reward}\n\n"
        "Return only the revised instruction for the coding agent."
    )
...
def create_llm_code_prompt(instruction: str) -> str:
    return (
        f"Instruction:\n{instruction}"   
        "Requirements:\n"
        "- Output only the Dafny code.\n"
        "- Return the full code in the following form:\n"
        "```dafny\n"
        "Dafny Code\n"
        "```\n"
        "- Make sure the code is free from syntax, type, undefined-variable, logic, and out-of-bounds issues and avoid recursion in the code.\n"
        "- Include appropriate specifications, invariants, assertions, and termination arguments if needed.\n\n"
...
def initialize_trained_slm():
    global GLOBAL_SLM_MODEL, GLOBAL_SLM_TOKENIZER

    if GLOBAL_SLM_MODEL is not None and GLOBAL_SLM_TOKENIZER is not None:
        return GLOBAL_SLM_MODEL, GLOBAL_SLM_TOKENIZER

    torch.cuda.empty_cache()
    os.makedirs(OFFLOAD_FOLDER, exist_ok=True)

    logging.info(f"Loading tokenizer: {SLM_MODEL_NAME}")
    ...
    logging.info(f"Loading base model: {SLM_MODEL_NAME}")
    ...
    logging.info(f"Loading trained LoRA adapter from: {SLM_LORA_PATH}")
    ...
    GLOBAL_SLM_MODEL = model
    GLOBAL_SLM_TOKENIZER = tokenizer
    return GLOBAL_SLM_MODEL, GLOBAL_SLM_TOKENIZER


def initialize_checkpoint_slm():
    global GLOBAL_SLM_MODEL, GLOBAL_SLM_TOKENIZER
...
def initialize_checkpoint_with_lora():
    torch.cuda.empty_cache()
    os.makedirs(OFFLOAD_FOLDER, exist_ok=True)
    ...
    # STEP 2: load checkpoint weights ON TOP
    ckpt = torch.load(CHECKPOINT_PATH, map_location="cpu")
...
def run_trained_slm(model, tokenizer, prompt: str) -> str:
    try:
        generation_config = {
            "max_new_tokens": 128,
            "do_sample": False,
        ...
        response = tokenizer.decode(generated_only, skip_special_tokens=True).strip()

        timestamp = time.strftime("%Y%m%d_%H%M%S")
        save_dir = os.path.join(OUTPUT_DIR, "slm_interactions")
        os.makedirs(save_dir, exist_ok=True)
        save_json(
            os.path.join(save_dir, f"slm_interaction_{timestamp}.json"),
            {
                "timestamp": timestamp,
                "prompt": prompt,
                "response": response,
        ...
        logging.error(f"Error in run_trained_slm: {str(e)}")
        return ""

def run_llm(prompt: str, model_name: str, last_code: str = "", last_error: str = "") -> Tuple[str, int]:
    try:
        full_instruction = prompt
        if last_code or last_error:
            prefix = "### Previous Attempt Context"
            if last_code:
                prefix += "\n### Previous Dafny Code\n" + last_code.strip() + "\n\n"
            if last_error:
                prefix += "\n### Previous Error\n" + last_error.strip() + "\n\n"
            prefix += "\nFix the issue and return the corrected code based on the following instructions:\n"
            full_instruction = prefix + prompt

        full_prompt = create_llm_code_prompt(full_instruction)
        ...
        if model_name.startswith("gpt-5"):
            ...
            response = openai_client.responses.create(
                model=model_name,
                instructions="You are an expert Dafny programmer.",
                input=full_prompt,
            )
            ...
            response = deepseek_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert Dafny programmer. You are given a description of a problem and you need to write a Dafny program to solve it."
                    },
                    {"role": "user", "content": full_prompt},
                ],
                temperature=0.2,
        ...
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        save_dir = os.path.join(OUTPUT_DIR, f"llm_interactions_{model_name.replace('/', '_')}")
        os.makedirs(save_dir, exist_ok=True)
        save_json(
            os.path.join(save_dir, f"llm_interaction_{timestamp}.json"),
            {
                "timestamp": timestamp,
                "model": model_name,
                "prompt": full_prompt,
        ...
        return (text.strip() if text else "", prompt_tokens)
...
def run_dafny(code: str, tmp_path: str, error_path: str) -> Tuple[int, str, str]:
    ...
    try:
        ...
        try:
            output, _ = process.communicate(timeout=DAFNY_TIMEOUT_SEC)
        except subprocess.TimeoutExpired:
            process.kill()
            process.communicate()
            output = f"Error: Dafny verification timeout ({DAFNY_TIMEOUT_SEC} seconds)"
...
def process_folder_experiment(
    ...
    short_prompt = create_short_prompt(task)
    slm_response = run_trained_slm(slm_model, tokenizer, short_prompt)
    llm_response, prompt_tokens = run_llm(slm_response, llm_model_name)
    code = extract_dafny_code(llm_response)
    success, output, final_code = run_dafny(code, tmp_path, error_path)
    ...
    for n in range(2, MAX_FEEDBACK_ATTEMPTS + 1):
        ...
        slm_response = run_trained_slm(slm_model, tokenizer, error_prompt)
        llm_response, prompt_tokens = run_llm(
            slm_response,
            llm_model_name,
            last_code=attempts[-1]["code"],
            last_error=attempts[-1]["output"],
        )
        code = extract_dafny_code(llm_response)
        success, output, final_code = run_dafny(code, tmp_path, error_path)
...
def main():
    logging.info("Initializing trained LoRA SLM...")
    slm_model, tokenizer = initialize_trained_slm()
    # slm_model, tokenizer = initialize_checkpoint_slm()
    # slm_model, tokenizer = initialize_checkpoint_with_lora()
