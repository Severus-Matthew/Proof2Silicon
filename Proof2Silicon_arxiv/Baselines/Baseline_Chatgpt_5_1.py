...
def RUN_LLM(client, subfolder, one_line_desc, detailed_desc, temperature, max_iterations=5):
    ...
    for i, prompt in enumerate(prompts, start=1):
        for attempt in range(max_iterations):
            ...
            promptfile = os.path.join(subfolder, f"Prompt_chatgpt5_1codex_0.25_{i}_{attempt}.dfy")
            output_dafny_path = os.path.join(subfolder, f"generated_chatgpt5_1codex_0.25_{i}_{attempt}.dfy")
            Error_path = os.path.join(subfolder, f"error_chatgpt5_1codex_0.25_{i}_{attempt}.txt")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--base_folders",
        nargs="+",
        default=["/mnt/shared/gpfs/home/manvij2/UIUC_PROJ_2/dataset/Dafny_gpt5_1codex"],
