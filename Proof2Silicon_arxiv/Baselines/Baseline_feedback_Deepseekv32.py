        ...
        if os.path.exists(one_line_file) and os.path.exists(detailed_file):
            ...
            for i in range(1, 6):  # For each prompt type
                for attempt in range(5):  # For each attempt
                    prompt_file = os.path.join(subdir, f"Prompt_Deepseekv32_0.25_{i}_{attempt}.dfy")
                    output_file = os.path.join(subdir, f"generated_Deepseekv32_0.25_{i}_{attempt}.dfy")
                    error_file = os.path.join(subdir, f"error_Deepseekv32_0.25_{i}_{attempt}.txt")
...
def RUN_LLM(subfolder, one_line_desc, detailed_desc, temperature, max_iterations=5):
    ...
    for i, prompt in enumerate(prompts, start=1):
        ...
        for attempt in range(max_iterations):
            ...
            promptfile = os.path.join(subfolder, f"Prompt_Dqwen32b_0.25_{i}_{attempt}.dfy")
            output_dafny_path = os.path.join(subfolder, f"generated_Deepseekv32_0.25_{i}_{attempt}.dfy")
            Error_path = os.path.join(subfolder, f"error_Dqwen32b_0.25_{i}_{attempt}.txt")
            ...
            messages = [
                {"role": "system", "content": "You are an Expert in Dafny programming, follow the below instrution to write an error free Dafny code, you can use the above examples to learn the style of writing Dafny code:"},
                {"role": "user", "content": prompt},
            ]
            outputs = pipe(messages, max_new_tokens=8000, do_sample=True, temperature=temperature, top_k=50, top_p=0.95)
...
if __name__ == "__main__":
    ...
    process_subfolders("/mnt/shared/gpfs/home/manvij2/UIUC_PROJ_2/dataset/Deepseekv32", temperature)
