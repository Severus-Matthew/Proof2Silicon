        ...
        if os.path.exists(one_line_file) and os.path.exists(detailed_file):
            ...
            for i in range(1, 6):  # For each prompt type
                for attempt in range(5):  # For each attempt
                    prompt_file = os.path.join(subdir, f"Prompt_chatgpto1_0.25_{i}_{attempt}.dfy")
                    output_file = os.path.join(subdir, f"generated_chatgpto1_0.25_{i}_{attempt}.dfy")
                    error_file = os.path.join(subdir, f"error_chatgpto1_0.25_{i}_{attempt}.txt")
...
def RUN_LLM(subfolder, one_line_desc, detailed_desc, temperature, max_iterations=5):
    ...
    for i, prompt in enumerate(prompts, start=1):
        ...
        for attempt in range(max_iterations):
            ...
            promptfile = os.path.join(subfolder, f"Prompt_chatgpto1_0.25_{i}_{attempt}.dfy")
            output_dafny_path = os.path.join(subfolder, f"generated_chatgpto1_0.25_{i}_{attempt}.dfy")
            Error_path = os.path.join(subfolder, f"error_chatgpto1_0.25_{i}_{attempt}.txt")
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {"role": "user", "content": "You are a helpful assistant. " + prompt}  # Combine system instructions into user message
                ],
            )

            # Extract and print the response content
            generated_content = response.choices[0]
...
if __name__ == "__main__":
    ...
    process_subfolders("/mnt/shared/gpfs/home/manvij2/UIUC_PROJ_2/dataset/CHATGPTo1", temperature)
